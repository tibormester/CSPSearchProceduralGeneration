...
**Conclusion:** The project will provide a foundation for the development of diverse sets of assets utilizing CSP techniques, enabling immersive and coherent game worlds with increased variety at reduced design complexity. Additionally, simulation of the world can be optimized by represention through evolution of the system instead of simulating individual actions of the actors in the system. The dynamic simulation of the appropriate level of detail will enable an infinite levels of detail for large scale worlds.
...
Infinite Variables and Domains: For large complex objects it may be desirable for a random number of objects to have random positions or more generally to have a random value from an infinite domain. We propose two solutions to reformat these desires to fit our framework:
...
Procedural Object: a procedural object has three parts, a template object (definition of variables and domains), a csp graph (formulation of the search problem as a list of constraints), and instances (solutions). For example, in our forest, we desire the generation of procedural plants, specifically trees and shrubs. The template object describes all plant objects defining things like location, where to draw leaves, trunk, and branches in a unified format. Next the csp graph has constraints such that searching for a tree yields something with a tall trunk and a lot of large branches while searching for a shrub yields the opposite. And lastly, when we get our solutions and wish to visualize the results, we can pass our soltions into the plant object model to view converter.

Variables and Domains: To avoid errors with constraints, it is enforced that all values in a domain are of the same type and that variables are similarily typed. This enables the instancing of constraints that utilize properties of these types in their evaluation. i.e. a location variable if typed as a vector3 must have a domain with only vector3 values so that constraints can be assured that they deal with only vector3 values. Variables are propeties of the procedural objects while a domain is the set of all possible values of the property.
...
Complex Objects: Complex objects are objects that can evolve, be simulated, or be partially assigned. 

Evolution: Evolution of an object is done by using the current state of the object as an initial solution assignment and use the updated set of constraints including the set of evolution constraints. The evolution constraints are constraints that dictate how an objects property should change over an evolution step and are likely dependent on the current state. For example a predator prey population constraint may require that the new prey propulation is less than the current prey population by a predation coefficient times the current predator population. 

Simulation: If our object changes over time, define an evolution csp that is the object's csp plus evoution constraints describing how the object changes over a time step. Then to simulate the system we iteratively solve the evolution csp, feeding the result back into itself. 

Partial Assignment: For a large complex object like an forest or a city, solving the object completely may be time consuming and uneccessary. If a player only ventures into a small portion of a forest, there is no need to determine what the rest of the forest looks like. So with layers, we can partition them into local graphs and solve them as independent layers. Furthermore we can delay determination of these layers until convenient or necessary. What this could look like is determining the location of every tree in the forest but delaying determining the layout of branches until the tree is within render distance of the player. Alternatively the forest may have a specific quadrant the player is in with the trees' locations determines, while the remaining quadrants are still lacking a solution. In general for randomly distributed objects, one can easily partion the layer and do the distribution on the partition. If the distribution isn't random and homogenous, then the placement of objects should be done at once and the determination of the objects done as needed.

**Proposed Layers of Problems**
Complex Procedural Object Forest:
Properties: 
...
Complex Procedural Object Population:
...
Simple Procedural Object Entity:
